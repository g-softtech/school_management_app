    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">Communicate with teachers and school administration</p>
        </div>
        ...
        <div className="w-64 flex-shrink-0 border-r border-secondary-100 flex flex-col">
          <div className="p-3 border-b border-secondary-100">
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search contacts…" className="input-field pl-8 py-1.5 text-sm w-full" />
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!activeContact ? (
            ...
            <>
              ...
              <div className="flex-1 min-w-0 overflow-y-auto p-4 space-y-3 bg-secondary-50/50">
                {thread.length === 0 && (
                  <div className="text-center py-10">
                    <FiMessageSquare size={32} className="mx-auto mb-2 text-secondary-300" />
                    <p className="text-secondary-400 text-sm">No messages yet. Say hello!</p>
                  </div>
                )}
                {thread.map((msg) => {
                  ...
                  return (
                    <div key={msg._id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl text-sm shadow-sm ${mine ? 'bg-primary-500 text-white rounded-br-sm' : 'bg-white text-secondary-800 rounded-bl-sm border border-secondary-100'}`}>
                        <p>{msg.content}</p>
      ...
      {/* User picker — parents can message teachers + admins */}
      <UserPickerModal
        isOpen={showPicker}
